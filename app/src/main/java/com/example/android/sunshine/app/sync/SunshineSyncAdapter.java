...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements
    ...
    // public static final int SYNC_INTERVAL = 60; - testing purpose
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            for(int i = 0; i < weatherArray.length(); i++) {
                ...
                if ((i == 0) && (!(mGoogleApiClient == null)) && mGoogleApiClient.isConnected()) {

                    Bitmap imageBitmap = BitmapFactory.decodeResource(getContext().getResources(),
                            Utility.getIconResourceForWeatherCondition(weatherId));
                    Asset bitmapAsset = createBitmapAsset(imageBitmap);

                    PutDataMapRequest mapRequest = PutDataMapRequest.create("/weather");
                    mapRequest.getDataMap().putAsset("weather_icon", bitmapAsset);
                    mapRequest.getDataMap().putString("high_temperature",
                            Utility.formatTemperature(getContext(), high));
                    mapRequest.getDataMap().putString("low_temperature",
                            Utility.formatTemperature(getContext(), low));

                    // - testing purpose for onDataChanged to trigger every time
//                    mapRequest.getDataMap().putLong("time", System.currentTimeMillis());
    ...
    private static Asset createBitmapAsset(Bitmap bitmap) {
